...
> **最後更新**：2026-01-19
> **整體進度**：100%
> **目前階段**：✨ AI 智慧排程功能開發（Phase 1 MVP 完成）
...
### AI 智慧排程 Phase 1 MVP（已完成）
- [x] S-001: 資料庫欄位擴展
  - [x] 新增 estimated_minutes 欄位（預估時間）
  - [x] 新增 task_type 欄位（focus/background）
  - [x] 更新 TypeScript 類型定義
- [x] S-002: 未排程任務 API
  - [x] `/api/tasks/unscheduled` 端點
  - [x] 支援篩選：priority、dueBefore、projectId
  - [x] 回傳包含專案名稱
- [x] S-003: 可用時段 API
  - [x] `/api/calendar/available-slots` 端點
  - [x] 整合 Google Calendar 取得忙碌時段
  - [x] 計算工作時間內的可用區間
- [x] S-004: AI Function Calling 架構
  - [x] `src/lib/ai-functions/` 模組架構
  - [x] 7 個 AI Functions 定義
  - [x] Function executor 路由邏輯
  - [x] 整合到 `/api/chat/stream` 支援 tool_calls
- [x] S-005: AI 預估時間
  - [x] GPT-4.1-mini 智慧預估
  - [x] 任務類型判斷（focus/background）
  - [x] 批次預估優化（單一 API 呼叫）
  - [x] 關鍵字回退邏輯
- [x] S-006: 排程演算法
  - [x] 智慧排程演算法實作
  ...
  - [x] 時段分配與更新邏輯
- [x] S-007: UI 排程預覽
  - [x] `SchedulePreview` 元件
  - [x] 按日期分組顯示
  - [x] 信心度與任務類型標示
  - [x] `/api/tasks/apply-schedule` API
- [x] S-008: 對話式排程整合（瀏覽器驗收）
  - [x] 3 輪瀏覽器驗收測試通過
  - [x] 修復 userId 未傳送導致 Function Calling 不觸發
  - [x] 修復套用排程時 userId 取得方式
  - [x] 完整流程：排程 → 預覽 → 套用 ✅

### AI 排程偏好學習 S-009（已完成）
- [x] S-009-1: 建立 scheduling_preferences 資料表
  - [x] 建立 `scheduling_preferences` 表（工作時間、午休、專注時段、每日限制）
  - [x] 建立 `scheduling_preference_logs` 表（學習記錄）
  - [x] RLS 政策與索引
- [x] S-009-2: 建立 TypeScript 類型與 API 層
  - [x] 完整類型定義（SchedulingPreferences、WeeklySchedule 等）
  - [x] API 函數（getSchedulingPreferences、updateSchedulingPreferences）
  - [x] 偏好解析函數（parsePreferenceFromInstruction）
  - [x] 時段評分函數（calculateSlotScore、isDayEnabled）
- [x] S-009-3: 修改排程演算法讀取偏好
  - [x] 載入用戶偏好到排程演算法
  - [x] 套用工作時間、每日上限、緩衝時間
  - [x] 過濾停用的日期和午休時段
- [x] S-009-4: 偵測對話中的偏好指令
  - [x] 建立 learnPreference.ts handler
  - [x] 整合到 chat/stream route
  - [x] 前端處理 preference_learned 事件
- [x] S-009-5: 記錄用戶排程修改行為
  - [x] 建立 learnFromScheduleAction.ts handler
  - [x] 記錄套用排程（正向回饋）
  - [x] 記錄取消排程（負向回饋）

### AI 排程衝突偵測 S-010（已完成）
- [x] S-010-1: 衝突偵測邏輯
  - [x] 建立 `conflictDetection.ts` 工具模組
  - [x] 實作 `checkTaskConflicts` 函數（單一任務衝突檢測）
  - [x] 實作 `checkScheduleConflicts` 函數（批次衝突檢測）
  - [x] 支援 5 種衝突類型（full_overlap, partial_start, partial_end, task_contains_event, event_contains_task）
  - [x] 格式化衝突訊息函數
- [x] S-010-2: 修改排程演算法自動避開衝突
  - [x] 整合 `getAvailableSlots` 的 `busyEvents` 資料
  - [x] 排程後執行衝突驗證
  - [x] 回傳 `conflictCheck` 和 `conflictSummary`
- [x] S-010-3: 衝突提示 UI
  - [x] 更新 `PendingSchedulePreview` 介面
  - [x] SchedulePreview 標題顯示衝突狀態徽章
  - [x] 個別任務卡片顯示衝突警告
  - [x] 衝突詳情展開（顯示衝突的行事曆事件）
- [x] S-010-4: 測試與驗收
  - [x] TypeScript 編譯通過
...
### 自然語言排程 S-011（已完成）
- [x] S-011-1: 自然語言日期解析
  - [x] 建立 `dateParser.ts` 日期解析工具
  - [x] 支援「今天」「明天」「後天」
  - [x] 支援「本週」「下週」「這週末」「下週末」
  - [x] 支援「這個月」「下個月」
  - [x] 支援「未來 N 天」格式
- [x] S-011-2: 擴展 AI Function 參數
  - [x] 更新 `generateSmartSchedule` 函數描述
  - [x] 加入自然語言日期範例說明
  - [x] 參數描述強調日期轉換
- [x] S-011-3: 更新系統提示詞
  - [x] 動態計算今天、本週、下週日期
...
- [x] S-011-4: 測試與驗收
  - [x] TypeScript 編譯通過
  - [x] Dev server 運作正常
  - [x] 擴展 `isSchedulingRelated` 關鍵字

### 排程調整功能 S-012（已完成）
- [x] 發現拖曳調整功能已在行事曆頁面實作
  - [x] handleDragStart、handleDragMove、handleDragEnd 函數
  - [x] 移動模式和調整大小模式
  - [x] 觸控裝置長按支援
  - [x] 拖曳時視覺回饋 tooltip
  - [x] Undo 功能與 toast 通知
...
### 排程報表 S-013（已完成）
- [x] S-013-1: 設計報表資料結構
  - [x] `src/lib/reports/types.ts` - 完整類型定義
  - [x] DailyStats、WeeklyReport、MonthlyReport 介面
  - [x] ChartDataPoint、ReportSummaryCard 介面
- [x] S-013-2: 建立排程統計 API
  - [x] `src/lib/reports/calculations.ts` - 計算函數
  - [x] calculateDailyStats、calculateWeeklyReport、calculateMonthlyReport
  - [x] generateSummaryCards、generateDailyBarChartData、generatePriorityChartData
  - [x] `/api/reports/weekly` API 端點
- [x] S-013-3: 建立報表 UI 元件
  - [x] `/app/reports/page.tsx` 報表頁面
  - [x] SummaryCard、DailyBarChart、PriorityDistribution 元件
  - [x] BestDayCard、ProjectDistribution 元件
  - [x] 週選擇器（上一週/下一週/回到本週）
  - [x] 趨勢比較（與上週對比）
- [x] S-013-4: 導航整合與測試
  - [x] 側邊欄導航連結（/reports）
...
- [x] `supabase/migrations/20260119_scheduling_preferences.sql` - 2026-01-20 執行完成
...
### 其他
3. 測試 AI 學習系統效果（Few-shot Learning）
4. 收集更多使用者互動回饋
5. 使用者體驗優化
